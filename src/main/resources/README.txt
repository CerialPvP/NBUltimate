------------------ NBULTIMATE README ------------------
Hello, and thank you for downloading NBUltimate on your Minecraft server.
This plugin allows you to play music files, such as NBS (Note Block Song), MIDI, MCSP2 (legacy format) and NoteBot.

------------------ DEFAULT SONGS ------------------
The songs in your songs folder are the default songs that come with your NBUltimate installation.
You may use them to test out the plugin, and see if everything works.
I (Cerial) would like to thank the following authors for providing the following songs to be included in your NBUltimate installation:

StunningWheel94:
    - Boss Battle 3o
    - Brass Blocks
    - Deal 'Em Out - Undertale Yellow
    - Stardrop Saloon - Stardew Valley
    - Deltarune - My Castle Town

Vizeon:
    - Escaping The Supernova

ShinkoNet:
    - Megalovania
    - The Ground's Color is Yellow

------------------ SUPPORT LINKS ------------------
If you ever need support, you may visit the following links:
- Discord server (general support and updates) discord.gg/8EX7SfMdGG
- GitHub repo (reporting issues, downloading stable/dev builds): https://github.com/CerialPvP/NBUltimate